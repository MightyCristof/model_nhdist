...
if keyword_set(xcm) then begin
    for i = 0,n_elements(engv)-1 do begin
        ...
        openw,1,'model_'+pref+'_'+model+'_scat'+(strsplit(scat,'.',/extract))[-1]+'.xcm' 
        printf,1,'abund angr'
        printf,1,'cosmo 70 0 0.73'
        printf,1,'method leven 10 0.01'
        printf,1,'xsect vern'
        printf,1,'xset delta 0.01'
        printf,1,'set id [open spectra_'+pref+'_'+model+'_scat'+(strsplit(scat,'.',/extract))[-1]+'.dat a]'
...
if keyword_set(calc) then begin
    files = file_search('spectra_*_'+model+'_scat'+(strsplit(scat,'.',/extract))[-1]+'.dat')
